...
from circa_rt.periodic_pipeline import PHASE5_TRACE_TYPES, generate_periodic_pipeline_trace
...
ATTACK_TRACE_TYPES = [t for t in PHASE5_TRACE_TYPES if t not in {"nominal", "mode_shift"}]
...
def build_trace(trace_type: str, seed: int, cfg: dict) -> pd.DataFrame:
    return generate_periodic_pipeline_trace(
...
def main() -> None:
    ...
    protocol = "phase5_periodic_pipeline_benign_calibration_split"

    for fold in range(int(cfg["n_seeds"])):
        test_seed = int(cfg["random_seed"]) + fold
        cal_seed = int(cfg["random_seed"]) + 3000 + fold
        ...
        for trace_type in PHASE5_TRACE_TYPES:
            ...
            summaries.append(
                add_summary_metadata(
                    summarize_detection(circa_result, trace_name=trace_key).to_dict(),
                    fold=fold,
                    protocol=protocol,
                    trace_type=trace_type,
                    test_seed=test_seed,
            ...
            if trace_type == "coupled_timing_semantic_attack" and fold == 0:
            ...
            for method, func in SPLIT_BASELINE_FUNCS.items():
                ...
                out_dir = raw_dir / trace_key / method
                out_dir.mkdir(parents=True, exist_ok=True)
                result.to_csv(out_dir / "scored.csv", index=False)
                summaries.append(
                    add_summary_metadata(
                        summarize_detection(result, trace_name=trace_key).to_dict(),
                        fold=fold,
                        protocol=protocol,
                        trace_type=trace_type,
                        test_seed=test_seed,
    ...
    bootstrap_ci_table(summary, ["method"], METRIC_COLS).to_csv(table_dir / "main_table_ci.csv", index=False)
    ...
    bootstrap_ci_table(summary, ["trace_type", "method"], METRIC_COLS).to_csv(table_dir / "scenario_table_ci.csv", index=False)

    attack_table = mean_table(summary[summary["trace_type"].isin(ATTACK_TRACE_TYPES)], ["method"], METRIC_COLS).sort_values(
    ...
    if representative_raw is not None and representative_scored is not None:
        plot_trace_scores(representative_raw, representative_scored, figure_dir / "phase5_circa_rt_score_timeline.png")

    print(f"wrote Phase 5 traces: {trace_dir}")
    print(f"wrote Phase 5 summary: {summary_dir / 'summary_all.csv'}")
    print(f"wrote Phase 5 main table: {table_dir / 'main_table.csv'}")
