...
def main() -> None:
    ...
    trace_dir = root / "traces_torchvision"
    raw_dir = root / "raw_torchvision"
    ...
    for model_name in models:
        ...
        for path in sorted(trace_dir.glob(f"{model_name}_*.csv")):
            ...
            row.update({"model": model_name, "trace_type": trace_type, "protocol": "autodl_torchvision_nominal_calibration"})
    ...
    summary.to_csv(summary_dir / "torchvision_summary_all.csv", index=False)
    summary.to_csv(table_dir / "torchvision_summary_all.csv", index=False)
    ...
    main.to_csv(table_dir / "torchvision_main_table.csv", index=False)
    scenario.to_csv(table_dir / "torchvision_scenario_table.csv", index=False)
    bootstrap_ci_table(summary, ["model", "method"], METRIC_COLS).to_csv(table_dir / "torchvision_main_table_ci.csv", index=False)
