...
def main() -> None:
    ...
    calibration = read_trace(trace_dir / "nominal.csv")
    ...
    for path in sorted(trace_dir.glob("*.csv")):
        ...
        out_dir = raw_dir / trace_type / "CIRCA-RT"
        out_dir.mkdir(parents=True, exist_ok=True)
        circa.to_csv(out_dir / "scored.csv", index=False)
        row = summarize_detection(circa, trace_name=trace_type).to_dict()
        row.update({"trace_type": trace_type, "protocol": "autodl_gpu_nominal_calibration"})
        summaries.append(row)
        for method, func in SPLIT_BASELINE_FUNCS.items():
            result = func(calibration, raw, baseline_cfg)
            out_dir = raw_dir / trace_type / method
            out_dir.mkdir(parents=True, exist_ok=True)
            result.to_csv(out_dir / "scored.csv", index=False)
            row = summarize_detection(result, trace_name=trace_type).to_dict()
            row.update({"trace_type": trace_type, "protocol": "autodl_gpu_nominal_calibration"})
            summaries.append(row)

    summary = pd.DataFrame(summaries)
    summary.to_csv(summary_dir / "summary_all.csv", index=False)
    summary.to_csv(table_dir / "gpu_trace_summary_all.csv", index=False)
    ...
    main_table.to_csv(table_dir / "gpu_trace_main_table.csv", index=False)
    scenario_table.to_csv(table_dir / "gpu_trace_scenario_table.csv", index=False)
    bootstrap_ci_table(summary, ["method"], METRIC_COLS).to_csv(table_dir / "gpu_trace_main_table_ci.csv", index=False)
